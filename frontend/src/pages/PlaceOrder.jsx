    ...
    return (
        <form onSubmit={handlePayment} className="w-full flex flex-col gap-4 mt-4">
            ...
            <button
                ...
                className="py-3 bg-yellow-400 text-black font-bold rounded-xl mt-2 active:scale-95 transform transition-transform duration-150"
            >
                {loading ? <Loading /> : 'Pay Now'}
            </button>
        </form>
...
function PlaceOrder() {
    ...
    const { serverUrl } = useContext(authDataContext); // ✅ same serverUrl for both COD & Stripe
    const [formData, setFormData] = useState({
        firstName: '',
        lastName: '',
        email: '',
    ...
    const inputClass =
        'w-full md:w-[48%] h-[50px] rounded-md bg-slate-700 placeholder:text-white text-white px-4 py-2';
        ...
        <div className="w-[100vw] min-h-[100vh] bg-gradient-to-l from-[#141414] to-[#0c2025] flex flex-col md:flex-row items-center justify-center gap-[50px] relative px-4 md:px-10 py-10">
            ...
            <div className="lg:w-[50%] w-[100%] flex items-center justify-center mt-[20px] lg:mt-0">
                <form
                    ...
                    className="lg:w-[70%] w-[100%] flex flex-col gap-4 bg-[#1a1a1a] p-6 rounded-3xl shadow-lg shadow-black/50"
                >
                    <Title text1="DELIVERY" text2="INFORMATION" />
                    ...
                    {method === 'cod' && (
                        <button
                            type="submit"
                            className="py-3 bg-blue-600 text-white rounded-xl mt-4 active:scale-95 transform transition-transform duration-150"
                        >
